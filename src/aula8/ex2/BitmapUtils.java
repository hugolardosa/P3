...
public class BitmapUtils {
	...
	public static void createSmaller(RandomAccessFile smaller) throws IOException {
		...
		// por cada linha image
		for(int j  = 0; j < Bitmap.bitmapInfoHeader.height; j += 2) {
	    		// adicional linha de pixels
	    		int i;
	    		for (i = 0; i < Bitmap.bitmapInfoHeader.width*3/2; i += 3) {
	    			Bitmap.data[i] = (Bitmap.data[(j * imgWidth) + i*2]);
	    			Bitmap.data[i] = (Bitmap.data[(j * imgWidth) + (i*2 + 1)]);
	    			Bitmap.data[i] = (Bitmap.data[(j * imgWidth) + (i*2 + 2)]);
	    		}
	    		// padding
	    		for (int k = 0; k < i%4; k++) {
